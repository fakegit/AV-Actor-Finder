...
TRAINING_FILE = './outputs/training_file.txt'
VALIDATION_FILE = './outputs/validate_file.txt'
...
def read_data_batch(csv_file_name, batch_size=FLAGS.batch_size):
    input_queue = get_input_queue(csv_file_name=csv_file_name)
    image, label, file_name = read_data(input_queue=input_queue)
    ...
    image = tf.image.random_contrast(image=image, lower=0.2, upper=2.0)
    image = tf.image.random_hue(image=image, max_delta=0.08)
    image = tf.image.random_saturation(image=image, lower=0.2, upper=2.0)
    ...
    batch_label_one_hot = tf.one_hot(tf.to_int64(batch_label),
...
def conv1(input_data):
    ...
    with tf.name_scope('conv_1'):
        ...
        h_conv1 = tf.nn.conv2d(input_data, W_conv, strides=[1,1,1,1], padding='SAME')
...
def build_model(images, keep_prob):
    ...
    r_dropout = tf.nn.dropout(r_fc2, keep_prob=keep_prob)
    print("shape after dropout with", keep_prob, "keep_prob :", r_dropout.get_shape())
